...
export default function CaesarCipher() {
  ...
  const encryptionData = textArray.map(pChar => {
    const pCode = pChar.charCodeAt(0) - 65;
    const cCodeTemp = isDecrypt ? pCode - shift : pCode + shift;
  ...
  return (
    <section className="view-section active">
      <div className="view-header">
        <h2>Caesar Cipher</h2>
        <p>The Caesar Cipher is one of the earliest and simplest methods of encryption technique. It's simply a type of substitution cipher, i.e., each letter of a given text is replaced by a letter some fixed number of positions down the alphabet. For example with a shift of 1, A would be replaced by B, B would become C, and so on. Mathematically, it operates under Modulo 26.</p>
      </div>
        ...
        <div className="glass-card">
          ...
          <div className="form-group">
            ...
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'5px'}}>
                ...
                <span style={{background:'rgba(255,255,255,0.1)', padding:'2px 8px', borderRadius:'4px', color:'var(--text-primary)', fontWeight:'600'}}>{shift}</span>
                <span style={{fontSize:'0.85rem', color:'var(--text-muted)'}}>25</span>
            </div>
            <input type="range" min="0" max="25" value={shift} onChange={e => { setShift(parseInt(e.target.value)); handleReset(); }} disabled={isPlaying} />
          </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                        ...
                        <motion.div key={`op-${currentStep}`} initial={{scale:0.95, opacity:0}} animate={{scale:1, opacity:1}} className="math-operation-box" style={{minHeight: '160px', width: '280px'}}>
                          <div style={{borderBottom: '1px solid var(--border-color)', paddingBottom: '8px', marginBottom: '8px'}}>
                            f(x) = (x {isDecrypt ? '-' : '+'} {shift}) mod 26
                          </div>
                          <div>'{currentData.pChar}' = <strong style={{color:'#fff'}}>{currentData.pCode}</strong></div>
                          <div>{currentData.pCode} {isDecrypt ? '-' : '+'} {shift} = <strong style={{color:'#fff'}}>{currentData.cCodeTemp}</strong></div>
