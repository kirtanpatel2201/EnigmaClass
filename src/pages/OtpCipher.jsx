...
export default function OtpCipher() {
  ...
  // Pad or truncate key to match text length
  let activeKey = key.toUpperCase().replace(/[^A-Z]/g, '');
  while (activeKey.length < textArray.length) {
    activeKey += String.fromCharCode(Math.floor(Math.random() * 26) + 65);
  }
  activeKey = activeKey.slice(0, textArray.length);

  const isDecrypt = mode === 'decrypt';
  
  const encryptionData = textArray.map((pChar, i) => {
    const pCode = pChar.charCodeAt(0) - 65;
    const keyChar = activeKey[i];
  ...
  const handleStart = () => {
    ...
    // Ensure key state is updated with the random padding if needed
    if (key.length < textArray.length) {
        setKey(activeKey);
    }
  ...
  const handleGenerateKey = () => {
      let rand = '';
      const len = textArray.length > 0 ? textArray.length : 10;
      for (let i = 0; i < len; i++) {
  ...
  return (
    <section className="view-section active">
      <div className="view-header">
        <h2>One-Time Pad</h2>
        <p>The one-time pad (OTP) is an encryption technique that cannot be cracked, but requires the use of a one-time pre-shared key the same size as, or longer than, the message being sent. In this technique, a plaintext is paired with a random secret key.</p>
      </div>
        ...
        <div className="glass-card">
          ...
          <div className="form-group">
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                <label style={{marginBottom: 0}}>Key Pad (Must match message length)</label>
                <button onClick={handleGenerateKey} style={{fontSize: '0.8rem', background: 'transparent', border: 'none', color: 'var(--accent-color)', cursor: 'pointer'}}>Generate Random</button>
            </div>
            <input type="text" placeholder="Random key sequence..." value={key} onChange={e => { setKey(e.target.value); handleReset(); }} disabled={isPlaying} style={{marginTop: '8px'}} />
          </div>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 10px', flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                        ...
                        <motion.div key={`op-${currentStep}`} initial={{scale:0.95, opacity:0}} animate={{scale:1, opacity:1}} className="math-operation-box" style={{minHeight: '160px', width: '280px'}}>
                          <div style={{borderBottom: '1px solid var(--border-color)', paddingBottom: '8px', marginBottom: '8px'}}>
                            f(x) = (x {isDecrypt ? '-' : '+'} Key) mod 26
                          </div>
                          <div>Pad Key Char = <strong style={{color:'#fff'}}>'{currentData.keyChar}' ({currentData.kCode})</strong></div>
                      ...
                      <div style={{display:'flex', flexDirection:'column', gap:'8px', alignItems:'center'}}>
                        <button className="btn-primary" style={{width: '100%', padding: '10px 16px', opacity: currentStep === textArray.length ? 0 : 1, pointerEvents: currentStep === textArray.length ? 'none' : 'auto'}} onClick={handleNext}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
                        </button>
                        {currentStep < textArray.length && (
                          <button 
                      ...
                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', width: '100%' }}>
                          <div>
                            ...
                            <div style={{display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'center'}}>
                              {textArray.map((char, i) => <div key={i} className="char-box">{char}</div>)}
                            </div>
                          </div>
